...
def _resampled_data(data, rsfreq, agg='sum'):
    ...
    resampled_data = data.resample(rsfreq).agg(getattr(pd.Series, agg), skipna=False)
...
class BaseRecording:
    ...
    def __init__(self, data, frequency, start_time=None, stop_time=None, period=None, mask=None):
        # Mandatory fields
        self._data = data
